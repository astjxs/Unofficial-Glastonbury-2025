...
// Parse the provided setlist data
export function parseSetlistData(setlistText: string): Artist[] {
  ...
  lines.forEach(line => {
    ...
    // Stage headers
    if (line.includes('Stage - ') || line.includes('Levels - ') || line.includes('Arcadia - ')) {
      const parts = line.split(' - ');
      currentStage = parts[0].toLowerCase()
        .replace(/\s+/g, '-')
        .replace('the-', '')
        .replace('-stage', '');
      currentDay = parts[1];
    ...
    if (line.includes(':') && (line.includes(' - ') || line.includes(' – '))) {
      ...
      const artistName = line.substring(0, colonIndex).trim();
      const timeInfo = line.substring(colonIndex + 1).trim();
      
      // Skip TBA and panel entries
      if (artistName === 'TBA' || artistName.startsWith('Panel:') || artistName.startsWith('Radical Round Up:')) return;
...
// Glastonbury 2025 setlist data
export const setlistData = `Glastonbury 2025
